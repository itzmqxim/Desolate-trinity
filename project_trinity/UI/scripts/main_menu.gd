...
func _on_fade_timer_timeout():
	if button_type == "start":
		if randf() < 0.5:  # 50% chance
			get_tree().change_scene_to_file("res://maps/map_office.tscn")
		else:
			get_tree().change_scene_to_file("res://maps/map_abstract.tscn")  # Your other scene
	elif button_type == "settings":
		get_tree().change_scene_to_file("res://reusables/settings.tscn")
